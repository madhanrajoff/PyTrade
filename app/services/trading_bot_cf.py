...
from boundary_objs.alpaca import AlpacaAPI
    ...
    def trade_bot(cls):
        ...
        try:
            if reqt["equity"]["EQ"]["isOpen"] is True:
                ...
                if alpaca.check_if_the_market_is_open_now():
                    ...
                    if len(positions) > 1:
                        for each in positions:
                            if each.market_value > each.cost_basis:
                                alpaca.submit_order(
                                    symbol=each.symbol,
                                    qty=each.qty,
                                    side="sell",
                        ...
                        profitable_quote = df[df.closePrice > df.bidPrice]
                        profitable_symbol = profitable_quote.min().symbol
                        alpaca.submit_order(
                            symbol=profitable_symbol,
                            qty=1,
                            side="buy",
                            type="market",
                            time_in_force="gtc"
                ...
                #
                # # Add to MySql
                # Df.to_sql(df=position_df, table_name="StrategyLog")
