...
class ConfigHelper:
    ...
    def environ_check(self, yaml_path):
        """
        Verifies the app's environment variables have been set.
        In other words, that 'source .env' has been run for the app
        """
        ...
        if self.APP_NAME is None:
            ...
            # ENV_APP_NAME = self.get_config_attrib('APP_NAME', option_val=-1, source=CfgSrc.ENV)
            assert YAML_APP_NAME == self.app_name_check, F'YAML app name should be {self.app_name_check}'
            # assert ENV_APP_NAME == self.app_name_check, F'Env app name should be: {self.app_name_check}'
    ...
    @classmethod
    def initialize(cls, config: 'ConfigHelper' = None):
        """
        - This method should be overridden by the concrete config class to
        initialize the app in an orderly fashion (libraries first, app sub-systems second)
